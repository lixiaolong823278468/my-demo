...
程序启动后、以及运行状态刷新时，系统会检查历史 Excel 文件的路径、大小和修改时间。如果历史数据没有变化，不会重复寻优和重训；如果检测到新增或修改，会自动启动训练窗口寻优任务。

自动寻优逻辑：

- 页面可配置 `寻优最大历史天数`，默认 `100` 天；自动启动时也会使用该默认值或上次保存的页面值
- 自动寻优区单独配置 `寻优验证集天数`、`寻优 XGBoost 轮数`、`局部细搜半径`
...
页面中的 `自动寻优训练窗口` 按钮可手动强制重新寻优；普通启动和状态刷新只会在历史数据变化时自动寻优。
