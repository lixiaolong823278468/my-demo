...
if not defined PYTHON_CMD (
  echo.
  echo launch_web failed: no usable Python executable was found.
  echo Tried:
  echo %TRIED_PYTHON%
  echo.
  echo Please install Python or repair the .venv environment before starting the web app.
  pause
  exit /b 1
)

echo Using Python: %PYTHON_CMD%
%PYTHON_CMD% api_server.py --open-browser
if errorlevel 1 (
  echo.
