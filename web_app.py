...
def render_header() -> None:
    render_template(
        ...
        subtitle=html.escape("XGBoost 直接价格预测 + 相似法参考 ｜ 手动重训 ｜ 版本切换 ｜ 结果回填 ｜ 实时运行日志 ｜ 价格区间约束"),
    )
...
def render_train_tab() -> None:
    ...
    st.markdown('<div class="hint-text">默认按最近 N 天滚动训练；也可以切换到手动日期范围。</div>', unsafe_allow_html=True)
    render_toolbar("训练参数设置", "当前前端资源已拆分到独立 HTML / JS / CSS 文件中，Python 仅负责加载模板和传递数据。", ["默认最近 60 天", "验证集 14 天", "保留版本与日志"])

    training_mode = st.selectbox("训练模式", ["按最近 N 天训练", "手动起止日期"], index=0)
    is_rolling_mode = training_mode == "按最近 N 天训练"
    col1, col2, col3, col4 = st.columns(4)
    training_window_days = col1.number_input("训练窗口天数", min_value=7, max_value=730, value=60, disabled=not is_rolling_mode)
    start_date = col2.date_input("训练起始日期", value=date(2025, 1, 1), disabled=is_rolling_mode)
    end_date = col3.date_input("训练结束日期", value=date(2026, 3, 31), disabled=is_rolling_mode)
    valid_days = col4.number_input("验证集天数", min_value=1, max_value=90, value=14)
    num_boost_round = st.number_input("训练轮数", min_value=50, max_value=3000, value=400, step=50)
    render_path_box(
        [
    ...
    if st.button("手动重训模型", type="primary", use_container_width=True):
        ...
        result = train_and_register(
            TrainConfig(
                ...
                training_window_days=int(training_window_days) if is_rolling_mode else None,
                num_boost_round=int(num_boost_round),
                start_date=None if is_rolling_mode else start_date.isoformat(),
                end_date=None if is_rolling_mode else end_date.isoformat(),
            ),
        ...
        metrics_df = pd.DataFrame(result.metrics).T.reset_index().rename(
            columns={
                "index": "分时段",
                "baseline_mae": "相似法参考MAE",
                "baseline_rmse": "相似法参考RMSE",
                "final_mae": "模型MAE",
                "final_rmse": "模型RMSE",
                "train_rows": "训练样本数",
                "valid_rows": "验证样本数",
...
def render_prediction_metrics(display_df: pd.DataFrame) -> None:
    model_price = display_df["模型预测价格"].astype(float)
    baseline_price = display_df["相似法参考价格"].astype(float)
    diff = display_df["模型与相似法差值"].astype(float)
    cards = [
        ("预测日期", str(display_df["日期"].iloc[0]), "本次输出的 96 点预测日期"),
        ("平均模型预测价格", f"{model_price.mean():.2f}", "96 点模型预测价格平均值"),
        ("最高模型预测价格", f"{model_price.max():.2f}", "96 点模型预测价格峰值"),
        ("最低模型预测价格", f"{model_price.min():.2f}", "96 点模型预测价格谷值"),
        ("相似法参考均价", f"{baseline_price.mean():.2f}", "相似法参考曲线的平均价格"),
        ("模型参考差均值", f"{diff.mean():.2f}", "模型预测价相对相似法参考价的平均差值"),
    ]
    cols = st.columns(3)
...
def render_prediction_chart(display_df: pd.DataFrame) -> None:
    line_df = display_df[["时段", "模型预测价格", "相似法参考价格"]].copy().melt("时段", var_name="曲线", value_name="价格")
    diff_df = display_df[["时段", "模型与相似法差值"]].copy()
    ...
    line_chart = (
        ...
        .encode(
            ...
            color=alt.Color("曲线:N", scale=alt.Scale(domain=["模型预测价格", "相似法参考价格"], range=["#1677ff", "#fa8c16"]), legend=alt.Legend(title=None)),
    ...
    diff_chart = (
        ...
        .encode(
            x=alt.X("时段:Q", title="时段"),
            y=alt.Y("模型与相似法差值:Q", title="模型与相似法差值"),
            tooltip=["时段", alt.Tooltip("模型与相似法差值:Q", format=".2f")],
        )
    )
    chart = (
        alt.layer(diff_chart, line_chart)
...
def render_predict_tab() -> None:
    ...
    render_range_tip("预测输出增加区间保护：模型预测价格低于 0 时按 0 输出，高于 1500 时按 1500 输出。")
    ...
    if st.session_state.last_predict_df is not None:
        ...
        render_chip_row(["蓝线：模型预测价格", "橙线：相似法参考价格", "绿色柱：模型与相似法差值"])
